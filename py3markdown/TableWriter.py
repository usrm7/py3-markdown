...
class TableWriter(object):
    def __init__(self, headers):
        ...
        self.column_sizes = array("i", list(map(len, headers)))
        self.data = []
    ...
    def add_row(self, row):
        ...
        if len_diff < 0:
            raise ValueError(
                "Cannot have more columns than headers (%d)!" % self.column_num
            )
        if len_diff > 0:
            row = row + len_diff * [""]
    ...
    def get_table(self):
        out = (
            u" | ".join(
                ("%%-%ds" % l) % h for h, l in zip(self.headers, self.column_sizes)
            )
            + "\n"
        )
        ...
        for row in self.data:
            out += (
                u" | ".join(("%%-%ds" % l) % h for h, l in zip(row, self.column_sizes))
                + "\n"
            )
        return out
